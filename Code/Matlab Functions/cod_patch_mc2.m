...
for i = 1:m
   for j = 1:m
      distance(i,j) = sqrt(((latm(i)-latm(j)).*(latm(i)-latm(j))) + ((longm(i)-longm(j)).*(longm(i)-longm(j))));
   end
end
...
out1(1,:) = n_patch(distance,cod_dens,t_increment,max_increment);

% Performs the analysis on the data.

for i = 2:100
   rnd_data = randomize_indiv_dens(cod_dens);
   out1(i,:) = n_patch(distance,rnd_data,t_increment,max_increment);
end
