PatchScale_Ages<-function(x,path,years,nafo,Species,convert=24950.144,SaveIm=TRUE){
  ...
  PatchExtent$Age[which(PatchExtent$Age=="a")]="5+"
  
  #Plot for all ages
  p=ggplot(PatchExtent,aes(x=Year,y=Patch))+geom_point()+geom_line()+facet_grid(Age~.)+theme_bw()+
    labs(y="Patch extent (km)",x="")+scale_x_continuous(breaks=min(years):max(years))+ 
  ...
  if(SaveIm){
    #Save picture
    png(filename = paste(path,"PatchScale_Age_",Speciesn,"_NAFO_",nafo,".png",sep=""), 
        width = 2400, height = 2400, res = 300, bg="transparent")
    
    print(p)
    grid.text(unit(0.95,"npc"),0.5,label = "Age (years)", rot = 270,vjust=-2.1)
    
    dev.off()
    print(paste("Patch Scale for Ages 1:4 and adults: ",path,"PatchScale_Age_",Speciesn,"_NAFO_",nafo,".png",sep=""))
  } else print(p)
