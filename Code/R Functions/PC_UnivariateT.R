PC_UnivariateT<-function(x,path,years,nafo,Species,age,convert=24950.144,TempData,log_y=FALSE,Print.cor=FALSE,SaveIm=TRUE){
...
TenData=cbind(TenData,TempData,rep("10 (km)",NROW(TempData)))
FiftyData=cbind(FiftyData,TempData,rep("50 (km)",NROW(TempData)))
HundData=cbind(HundData,TempData,rep("100 (km)",NROW(TempData)))
colnames(TenData)[9]="ID"
colnames(FiftyData)[9]="ID"
colnames(HundData)[9]="ID"

ComboData=rbind(TenData,FiftyData,HundData)
ComboData$ID=factor(ComboData$ID,levels=c("100 (km)","50 (km)","10 (km)"))
...
LabData$ID=factor(LabData$ID,levels=c("100 (km)","50 (km)","10 (km)"))
